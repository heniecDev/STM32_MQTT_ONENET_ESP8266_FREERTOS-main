...
/*          	WIFI网络与ONENET配置（配置）			      	   */
/*-------------------------------------------------------------*/
const char SSID[] 			 = "honor";          //路由器名称
const char PASS[] 			 = "h477477h";    //路由器密码

const char PRODUCTID[] 	     = "605110";  	   //产品ID
const char DEVICEID []	     = "1084960194";    //设备ID  
...
#define beepOn_flag = "1"		//蜂鸣器开
#define beepOff_flag = "0"	//蜂鸣器关
...
void my_mq2_task(void *pvParameters);
//LCD任务，显示屏
TaskHandle_t LCD_Task_Handler;
void my_lcd_task(void *pvParameters);

//BEEP任务，蜂鸣器
TaskHandle_t BEEP_Task_Handler;
void my_beep_task(void *pvParameters);
...
void my_start_task(void *pvParameters)
{
    ...
    xTaskCreate(my_sun_task, 			"my_sun_task",        		128, NULL, 3, &SUN_Task_Handler);	
		 //创建MQ2_ADC任务，气体传感器
    xTaskCreate(my_mq2_task, 			"my_mq2_task",        		128, NULL, 3, &MQ2_Task_Handler);	
		 //创建LCD任务，显示屏
    xTaskCreate(my_lcd_task, 			"my_lcd_task",        		128, NULL, 3, &LCD_Task_Handler);	
		
		
		//创建BEEP任务，蜂鸣器
		xTaskCreate(my_beep_task,			"my_beep_task",						128, NULL, 4, &BEEP_Task_Handler);
...
void my_mq2_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		sprintf(data_of_sensor, "\"ADC_CH1_VAL\":\"%d\",\"ADC_CH1_VOL\":\"%f\",", adcx, temp); 
//		sprintf(data_of_sensor, "\"ADC_CH1_VAL\":\"%d\",\"ADC_CH1_VOL\":\"%f\",",adcx,adcx2+temp);  		//电压
...
void my_lcd_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		//显示电压值  
		LCD_ShowxNum(156,150,adcx,1,16,0);//电压整数部分
		//printf("ADC_CH1_VOL:%f\r\n",adcx+temp);//串口输出电压
		temp-=adcx;
		temp*=1000;
		
		LCD_ShowxNum(172,150,temp,3,16,0x80);//电压小数部分	
		delay_ms(10*1000);//延时10s
	}
} 
...
void my_beep_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		//构建消息体
		//消息体："beep":"%d",
		sprintf(data_of_sensor, "\"beep_flag\":\"%d\",",0); //构建消息体的一部分
